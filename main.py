...
def main():
    ...
    if COMET_EXPERIMENT:
        ...
        saved_model, model = train(train_data, val_data, saver)
    with torch.no_grad():
        test_loss_model, preds_model = model(train_data.get_pyg_graph(device=FLAGS.device), test_data, -1)
    ...
    if COMET_EXPERIMENT:
        ...
        with COMET_EXPERIMENT.test():
            ...
            COMET_EXPERIMENT.log_confusion_matrix(y_true, y_pred, index_to_example_function=index_to_example, labels=list(test_data.label_dict.keys()))
